<div align="center">

# NEVA MOBILE

**Telefoncu için geliştirildi. Telefoncularla geliştiriliyor.**

İkinci el telefon alım satımı yapan işletmeler için geliştirilen modern, hızlı ve tamamen offline çalışan masaüstü yönetim yazılımı.

[![Latest Release](https://img.shields.io/github/v/release/coban9066/neva-mobile?label=s%C3%BCr%C3%BCm&color=2ea44f&style=for-the-badge)](https://github.com/coban9066/neva-mobile/releases/latest)
[![Platform](https://img.shields.io/badge/platform-Windows%2010%2F11-0078D6?style=for-the-badge&logo=windows11&logoColor=white)](https://github.com/coban9066/neva-mobile/releases/latest)
[![Offline](https://img.shields.io/badge/%C3%A7al%C4%B1%C5%9Fma-tamamen%20offline-16a34a?style=for-the-badge)](#)

<br>

[![Tauri](https://img.shields.io/badge/Tauri-24C8DB?style=flat-square&logo=tauri&logoColor=white)](#)
[![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)](#)
[![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)](#)
[![Rust](https://img.shields.io/badge/Rust-000000?style=flat-square&logo=rust&logoColor=white)](#)
[![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white)](#)

<br>

### ⬇️ [**NEVA MOBILE Setup.exe — Son Sürümü İndir**](https://github.com/coban9066/neva-mobile/releases/latest)

</div>

---

<br>

## ✨ Özellikler

<table>
<tr>
<td width="33%" valign="top">

**📥 Alış & Satış**
- Telefon Alış Yönetimi
- Telefon Satış Yönetimi
- IMEI Yönetimi
- Kâr Hesaplama

</td>
<td width="33%" valign="top">

**🛡️ Güvence**
- Garanti Takibi
- Masraf Takibi
- Kasa Yönetimi
- Lisans Sistemi

</td>
<td width="33%" valign="top">

**⚙️ Altyapı**
- Otomatik Güncelleme
- SQLite Veritabanı
- Tamamen Offline
- Cihaza Özel Lisans

</td>
</tr>
</table>

<br>

## 🎯 Kimler İçin?

<div align="center">

| 📱 Telefon Alım Satım Mağazaları | 🏪 GSM Bayileri | ♻️ İkinci El Satıcıları | 💼 Telefon Ticareti Yapan İşletmeler |
|:---:|:---:|:---:|:---:|

</div>

<br>

## 🆚 Neden NEVA MOBILE?

| | 📊 Excel / Defter | ✅ NEVA MOBILE |
|---|:---:|:---:|
| IMEI Takibi | ✗ | ✓ |
| Garanti Takibi | ✗ | ✓ |
| Otomatik Kâr Hesabı | ✗ | ✓ |
| Kasa Yönetimi | Manuel | ✓ Otomatik |
| Masraf Takibi | Dağınık | ✓ Telefon Bazlı |
| Otomatik Güncelleme | ✗ | ✓ |
| Offline Çalışma | ✓ | ✓ |
| Lisans / Cihaz Güvenliği | ✗ | ✓ |
| Telefoncuya Özel Tasarım | ✗ | ✓ |

<br>

## 🖼️ Ekran Görüntüleri

<div align="center">

<table>
<tr>
<td align="center" width="50%">

*Dashboard*
<br>
<img src="https://via.placeholder.com/480x300/1e293b/94a3b8?text=Dashboard" width="100%" />

</td>
<td align="center" width="50%">

*Telefonlar*
<br>
<img src="https://via.placeholder.com/480x300/1e293b/94a3b8?text=Telefonlar" width="100%" />

</td>
</tr>
<tr>
<td align="center" width="50%">

*Satış*
<br>
<img src="https://via.placeholder.com/480x300/1e293b/94a3b8?text=Sat%C4%B1%C5%9F" width="100%" />

</td>
<td align="center" width="50%">

*Kasa*
<br>
<img src="https://via.placeholder.com/480x300/1e293b/94a3b8?text=Kasa" width="100%" />

</td>
</tr>
</table>

</div>

<br>

---

## 📥 Kurulum

> **Sadece aşağıdaki dosyayı indiriniz:**
>
> ### ✅ `NEVA MOBILE Setup.exe`

Release sayfasında ayrıca `latest.json`, `.sig` ve *Source code* dosyaları görünür — bunlar uygulamanın **otomatik güncelleme sistemi** içindir. Normal kullanıcı bunları indirmemelidir, hiçbir işe yaramaz.

```
1. Setup.exe'yi indirin
2. Çift tıklayıp kurun
3. Cihaz Kimliğinizi kopyalayıp Instagram'dan gönderin
4. Lisans kodunuzu yapıştırıp kullanmaya başlayın
```

<br>

## 🔄 Otomatik Güncelleme

> Yeni sürüm çıktığında NEVA MOBILE bunu **otomatik algılar**. Tek tıkla güncellenir.

- ✔ Veritabanınız korunur
- ✔ Lisansınız korunur
- ✔ Ayarlarınız korunur

Hiçbir şey elle yapmanız gerekmez.

<br>

## 💳 Lisans

<div align="center">

### Tek Sefer Ödeme

✔ Süresiz Kullanım · ✔ Ücretsiz Güncellemeler · ✔ Lisans Transferi · ✔ Tamamen Offline

</div>

<br>

## 🚀 Deneme Lisansı

Programı indirip kurduktan sonra deneme lisansı almak için:

<div align="center">

**Instagram üzerinden yazın → [@prodbycoban](https://instagram.com/prodbycoban)**

</div>

<br>

## ❓ Sık Sorulan Sorular

<details>
<summary><b>Program internet ister mi?</b></summary>
<br>
Hayır. NEVA MOBILE tamamen offline çalışır. İnternet yalnızca otomatik güncelleme kontrolü için (isteğe bağlı) kullanılır.
</details>

<details>
<summary><b>Verilerim nerede tutuluyor?</b></summary>
<br>
Sizin bilgisayarınızda, kendi veritabanınızda. Başka kimse göremiyor, hiçbir sunucuya gönderilmiyor.
</details>

<details>
<summary><b>Bilgisayarımı formatlarsam ne olur?</b></summary>
<br>
Düzenli aldığınız yedekten geri yükleyebilirsiniz.
</details>

<details>
<summary><b>Lisans başka bir bilgisayara taşınabilir mi?</b></summary>
<br>
Evet — lisans transferi desteklenir.
</details>

<br>

## 💬 Geri Bildirim

Hata bildirimi, öneriler ve yeni özellik talepleri:

<div align="center">

**[@prodbycoban](https://instagram.com/prodbycoban)**

</div>

<br>

---

<div align="center">

### NEVA MOBILE

Telefoncu için geliştirildi.
Telefoncularla geliştiriliyor.

[@prodbycoban](https://instagram.com/prodbycoban)

© 2026 NEVA MOBILE

</div>
